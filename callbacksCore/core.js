//Inmutabilidad: Todos los datos deben ser tratados de manera inmutable. No debes modificar el arreglo original de videojuegos.
...
// Ej 1 : Un conjunto de videojuegos cuyo número de identificación es divisible uniformemente por 3. 
const juegosDivisiblesPor3 = videojuegos.filter(juego => juego.id % 3 === 0);
console.log(juegosDivisiblesPor3);

// Ej 2 :  Un conjunto de videojuegos que pertenecen al género “acción-RPG”.
const juegosRPG = videojuegos.filter(juego => juego.genero === "acción-RPG");
console.log(juegosRPG);

//ej 3: Un conjunto de videojuegos que tienen más de un género.
const generosMultiples = videojuegos.filter(juego => juego.genero.includes("-"));
console.log(generosMultiples);

//ej 4: Una lista con los nombres de los videojuegos.
const listadeNombres = videojuegos.map (juego => juego.nombre);
console.log (listadeNombres);

//ej 5 : Una lista con los nombres de los videojuegos con un número de identificación superior a 19.
const idMayor19 = videojuegos.filter(juego => juego.id > 19).map(juego => juego.nombre);
console.log(idMayor19);

//ej 6:  Una lista con los nombres de los videojuegos cuyo único género es “shooter”.
const juegosShooters = videojuegos.filter(juego => juego.genero === "shooter").map(juego => juego.nombre);
console.log(juegosShooters);

// ej 8:  Una lista que contenga solo el primer género de todos los videojuegos cuyo segundo género es “aventura”.
...
// ej 9:  Un conteo del número de videojuegos que son del género “party”.
const generoParty = videojuegos.filter (juego => juego.genero === "party").length;
console.log(generoParty);

// ej 10:  Una lista con todos los videojuegos excepto aquellos cuyo número de identificación sea múltiplo de 5.
const juegosNoMultiplo5 = videojuegos.filter(juego => juego.id % 5 !== 0);
console.log(juegosNoMultiplo5); 

// Una lista con todos los videojuegos y para el videojuego con el número de identificación 5, se cambia su género por “otro”.
const juegosGeneroModificado = videojuegos.map(juego =>
    juego.id === 5 ? { ...juego, genero: "otro" } : juego
);
console.log(juegosGeneroModificado);